...
namespace simulator_objects
{
	...
	glm::mat4 previous_mvp_matrix;
	glm::mat4 current_mvp_matrix;
	...
	void initialize()
	{
		...
		base_noise_scale_dataref = export_float_dataref("enhanced_cloudscapes/base_noise_scale", 0.000025f);
		detail_noise_scale_dataref = export_float_dataref("enhanced_cloudscapes/detail_noise_scale", 0.0002f);
		...
		cloud_height_datarefs[0] = export_float_dataref("enhanced_cloudscapes/cirrus/height", 2500.0f);
		cloud_height_datarefs[1] = export_float_dataref("enhanced_cloudscapes/scattered/height", 4000.0f);
		cloud_height_datarefs[2] = export_float_dataref("enhanced_cloudscapes/broken/height", 5000.0f);
		cloud_height_datarefs[3] = export_float_dataref("enhanced_cloudscapes/overcast/height", 5000.0f);
		cloud_height_datarefs[4] = export_float_dataref("enhanced_cloudscapes/stratus/height", 6000.0f);
		...
		cloud_coverage_datarefs[0] = export_float_dataref("enhanced_cloudscapes/cirrus/coverage", 0.375f);
	...
	void update()
	{
		...
		current_mvp_matrix = projection_matrix * modelview_matrix;
		...
		for (int layer_index = 0; layer_index < CLOUD_LAYER_COUNT; layer_index++) cloud_bases[layer_index] = XPLMGetDataf(cloud_base_datarefs[layer_index]);
		for (int type_index = 0; type_index < CLOUD_TYPE_COUNT; type_index++) cloud_heights[type_index] = XPLMGetDataf(cloud_height_datarefs[type_index]);
		...
		for (int type_index = 0; type_index < CLOUD_TYPE_COUNT; type_index++) cloud_coverages[type_index] = XPLMGetDataf(cloud_coverage_datarefs[type_index]);
		...
		for (int type_index = 0; type_index < CLOUD_TYPE_COUNT; type_index++) cloud_densities[type_index] = XPLMGetDataf(cloud_density_datarefs[type_index]);

		for (int layer_index = 0; layer_index < WIND_LAYER_COUNT; layer_index++) wind_altitudes[layer_index] = XPLMGetDataf(wind_altitude_datarefs[layer_index]);
		...
		local_time = XPLMGetDataf(local_time_dataref);
	}
}